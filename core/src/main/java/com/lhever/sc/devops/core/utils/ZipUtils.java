...
import com.lhever.sc.devops.core.support.logger.LogFactory;
...
//        List<String> names = new ArrayList<>(Arrays.asList("fag第三发放.jpg","ddd.jpg"));
    ...
    public static String nameNoPostfix(File srcFile) {
        ...
        int i = name.lastIndexOf(CommonConstants.DOT);
    ...
    private static String trimTail(String path) {
        ...
        path = path.trim();
        path = (path.endsWith(CommonConstants.SLASH) || path.endsWith(CommonConstants.BACK_SLASH))
    ...
    public static void unZip(File srcFile, String destDirPath) {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                String unzipPath = destDirPath + CommonConstants.SLASH + entry.getName();
                // 如果是文件夹，就创建个文件夹
