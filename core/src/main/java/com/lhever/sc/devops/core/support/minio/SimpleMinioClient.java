package com.lhever.sc.devops.core.support.minio;

import com.lhever.sc.devops.core.constant.CommonConstants;
import com.lhever.sc.devops.core.utils.FileTypeUtils;
...
public class SimpleMinioClient {
    ...
    public SimpleMinioClient(String host, Integer port, String username, String password) {
        try {
            minioClient = new MinioClient("http://" + host + CommonConstants.COLON + port, username, password);
     ...
     *
     * @return
     * @author jianghaitao6 2019/4/5 15:26
     * @modificationHistory=========================逻辑或功能性重大变更记录
     * @modify by user: {修改人} 2019/4/5 15:26
     * @modify by reason:{原因}
     */
    public void transferTo(MultipartFile file, String filePath) throws IOException {
        long startTime = System.currentTimeMillis();
        logger.info("fileName：" + file.getOriginalFilename());
        File newFile = new File(filePath + CommonConstants.SLASH + file.getOriginalFilename());
        //通过CommonsMultipartFile的方法直接写文件（注意这个时候）
        file.transferTo(newFile);
        long endTime = System.currentTimeMillis();
        logger.info("采用file.Transto的运行时间:{}ms", (endTime - startTime));
    }
