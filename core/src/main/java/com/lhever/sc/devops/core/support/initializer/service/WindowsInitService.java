package com.lhever.sc.devops.core.support.initializer.service;

import com.lhever.sc.devops.core.config.PropertiesReader;
...
public class WindowsInitService extends BaseInitService {
    ...
    private void changeLogDir(Element element, boolean hasPort) {
        ...
        builder.append("../logs/")
                ...
                .append(CommonConstants.UNDER_SCORE);
        if (hasPort) {
            builder.append("${port}");
        } else {
    ...
    private void doGenerateConf() {
        ...
        //属性文件转map
        PropertiesReader propertiesFile = new PropertiesReader(propPath, true);
...
//                .append("wrapper.exe uninstall ") //卸载同名服务
        ...
        //脚本内容写入文件
        FileUtils.writeToFileByNio(fileFullName, stWrapper.toString(), CommonConstants.CHARSET_UTF8);
