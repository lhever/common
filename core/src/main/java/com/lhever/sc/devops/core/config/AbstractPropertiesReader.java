...
public abstract class AbstractPropertiesReader {
    ...
    public boolean writeNoEscape(String path) {
        return writeNoEscape(props, path);
    }
    ...
    public boolean writeNoEscape(Properties props, String path) {
        ...
        try {
            ...
            writer = new OutputStreamWriter(os, CommonConstants.CHARSET_UTF8);
            ...
            for (Enumeration<?> e = props.keys(); e.hasMoreElements(); ) {
                ...
                if (props.containsKey(key)) {
                    ...
                    String stringVal = (val == null ? CommonConstants.EMPTY : val.toString());
                    if (StringUtils.isBlank(stringVal)) {
                        propList.add(key + "=" + CommonConstants.EMPTY + LINE_RETURN);
                    } else {
            ...
            bw.write(StringUtils.join(propList, CommonConstants.EMPTY));
            bw.flush();
        } catch (Exception e) {
            LOG.error("store properties failed", e);
            success = false;
    ...
    private static class PropertyKeyParser {
        ...
        public String getFirstAndSecond() {
            ...
            if (num >= 2) {
                return segment[0] + CommonConstants.DOT + segment[1];
            }
            return segment[0];
        }
